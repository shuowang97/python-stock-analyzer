...
async function thirdSearch() {
    ...
    let array_price = []
    let array_volume = []
    let price_min = Number.MAX_SAFE_INTEGER, price_max = 0, volume_min = Number.MAX_SAFE_INTEGER, volume_max = 0
    for(var i = 0; i < response.length; i++){
        ...
        volume_min = Math.min(volume, volume_min)
        volume_max = Math.max(volume, volume_max)
    ...
    extremeArr[2] = volume_min
    extremeArr[3] = volume_max
    ...
    // show panel
    createCarts(stockSymbol.toUpperCase(), today, array_price, array_volume, extremeArr)
    // getAxes()
...
function createCarts(symbol, date, data, volume, extremes) {
    // Highcharts.getJSON('https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/new-intraday.json', function (data) {
        ...
        if(extremes[1] * 0.7 > extremes[0]) {
            ...
            if(extremes[1] > 100){
                extremes[0] = extremes[0] * 0.8
            }else {
                extremes[0] = extremes[0] * 0.5
            }
        }
        // create the chart
        chart = Highcharts.stockChart('form3', {
            ...
            yAxis: [
                {
                    ...
                    opposite: false,
                    max: parseInt(extremes[1]),
                    min: parseInt(extremes[0])
                }, {
                    title: {
                        text: "Volume"
                    },
